...
gm<-ggplot(df_charts, aes(x = gm_this_year/10^3)) + 
  ...
  labs(title = "Distribution of Gross Margin", x= "Gross Margin (K$)", y="# partners")
...
gm_sales_sct<-ggplot(df_charts, aes(y = gm_this_year/10^3, x = sales_this_year/10^3)) + 
  ...
  labs(title = "Pattern of association between GM and Sales", x= "Sales (K$)", y="Gross Margin")

bp_grid <- plot_grid(gm,sales,gm_sales_sct, nrow = 2, ncol = 2)
...
data_w_clusters <- cbind(df_arr, 
                         data.frame("cluster" = factor(km$cluster)))

centers <- data.frame(km$centers)
# str(centers)
centers <- centers %>% mutate(cluster=factor("center", levels = c(1, 2, 3, "center")))
...
ggplot(data_w_clusters_centers, 
  ...
  labs(title="States by Murder and Urban Population", caption = "(Variables are scaled)")
```
